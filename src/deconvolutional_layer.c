...
void bilinear_init(layer l)
{
    ...
    for(f = 0; f < l.n; ++f){
        for(j = 0; j < l.size; ++j){
            for(i = 0; i < l.size; ++i){
                float val = (float) ((1 - fabs(i - center)) * (1 - fabs(j - center)));
                int c = f%l.c;
...
void backward_deconvolutional_layer(layer l, network net)
{
    ...
    for(i = 0; i < l.batch; ++i){
        ...
        float *ai = net.input + i*mi*ki;
        float *bi = net.workspace;
        float *ci = l.weight_updates;

        im2col_cpu(l.delta + i*l.outputs, l.out_c, l.out_h, l.out_w,
                l.size, l.stride, l.pad, bi);
        gemm_cpu(0,1,mi,ni,ki,1,ai,ki,bi,ki,1,ci,ni);
        ...
        if(net.delta){
            ...
            float *ad = l.weights;
            float *bd = net.workspace;
            float *cd = net.delta + i*nd*md;

            gemm_cpu(0,0,md,nd,kd,1,ad,kd,bd,nd,1,cd,nd);
        }
    }
}
